        ...
        <div className="flex flex-col justify-center p-8 lg:p-14">
          ...
          <form className="mt-8 space-y-4">
            {["Nama Lengkap", "Alamat Email", "Nama UMKM/Organisasi", "Kata Sandi", "Konfirmasi Kata Sandi"].map((label, index) => (
              <label key={label} className="block text-sm font-semibold text-[#001229]">{label}<input type={index > 2 ? "password" : "text"} placeholder={label} className="mt-2 w-full rounded-xl border border-slate-200 p-3.5 outline-none focus:border-[#0049A8]" /></label>
            ))}
