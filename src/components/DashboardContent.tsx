  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard title="Total Pendapatan" value="Rp 24.580.000" change="+12.5%" icon="💰" />
        <StatCard title="Total Pengeluaran" value="Rp 9.120.000" change="-4.3%" icon="💸" positive />
        <StatCard title="Laba Bersih" value="Rp 15.460.000" change="+18.2%" icon="📈" />
        <StatCard title="Total Transaksi" value="312 Transaksi" change="+8.1%" icon="🧾" />
      </div>
      ...
      <section className="rounded-2xl border border-[#E6EDF6] bg-white p-6 shadow-sm">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Statistik keuangan harian Anda</p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {stock.map(([name, qty, status]) => (
              <div key={name} className="flex items-center justify-between border-b border-slate-100 pb-3 last:border-0">
                ...
                <span className={`rounded-full px-3 py-1 text-xs font-bold ${status === "Kritis" ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600"}`}>
                  {qty} · {status}
                </span>
              </div>
            ))}
          ...
          <div className="space-y-4">
            {products.map(([name, qty], i) => (
              <div key={name} className="flex items-center gap-4">
                ...
                <span className="text-sm font-bold text-[#001229]">{qty}</span>
              </div>
            ))}
          </div>
        </section>
      ...
      <div className="flex items-center gap-4 rounded-2xl border border-red-100 bg-red-50 p-5">
        ...
        <div>
          ...
          <p className="text-sm text-red-600">4 pcs dan 12 pcs produk berada dalam kondisi kritis.</p>
        </div>
      </div>
    </div>
  );
